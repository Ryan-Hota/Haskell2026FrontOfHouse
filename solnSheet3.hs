...
rotCipher n = map (chr . go . ord) where
    go :: Int -> Int
    go x
        | x == 32   = 32
        | otherwise = 97 + ((x - 97) + n) `mod` 26
...
chunkDivider n l = if length l < n then [] else chunk : chunkDivider n excess where
    (chunk, excess) = splitAt n l
